define([
	...
	'/js/admin/news/collection.js',
	'/js/admin/news/view.js'
], function($, _, Backbone, webutils, UploadView, Banner, BannerList, BannerView) {
	...
	var AppView = Backbone.View.extend({
		...
		addHtml: function() {
			...
			$(this.el).find('#addnews-add').html(htmlResult);
			$(this.el).find('#addnews-add').find('[data-type="upload"]').html(new UploadView({}).el);
		},
		...
		addOne: function(banner) {
			banner.set({
				//content: webutils.slice(webutils.trimHtml(banner.get('content')), 50)
			});
			banner.set({
			...
			$('#addNewsTbody').append(view.render().el);
		},
		addAll: function() {
